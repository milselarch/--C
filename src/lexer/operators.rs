...
impl OperatorProcessor {
    ...
    pub fn get_operator(&self) -> Operators {
        // println!("Getting operator: {:?} {:?}", self.operator, self.pattern);
        self.operator
    }
}
...
impl TokenBuilder for OperatorProcessor {
    ...
    fn process_char(&self, c: char) -> ProcessResult {
        // println!("{}", format!("Processing {:?} char: '{}'", self, c));
...
pub struct OperatorsBuilder {
    ...
    pub(crate) operator_processors: Vec<OperatorProcessor>,
}
impl OperatorsBuilder {
    ...
    pub(crate) fn new() -> OperatorsBuilder {
        ...
        OperatorsBuilder {
            ...
            operator_processors: OperatorsBuilder::create_processors(patterns)
        }
    }
}
...
impl Display for OperatorsBuilder {
    fn fmt(&self, f: &mut Formatter<'_>) -> fmt::Result {
        write!(f, "OperatorsBuilder {:?}", self.operator_processors)
    }
}
...
impl TokenBuilder for OperatorsBuilder {
    ...
    fn _push_char(&mut self, char: char) {
        ...
        for processor in &mut self.operator_processors {
            ...
            } else if !process_result.complete {
                // println!("MARK_DONE {}", format!("{:?}", processor));
                processor.base_mut().set_invalid()
            }
        }
    }
